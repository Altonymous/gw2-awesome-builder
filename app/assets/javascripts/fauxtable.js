...
*
* fauxtable
* - a Modific of sorttable by David Brink & Andres Koetter
*
*/
(function ($) {
    $.widget("db.fauxtable", $.ui.sortable, {
        widgetEventPrefix: "fauxtable",
        options: {
            helper: "#stats",
            helperCells: 1
        },
        ...
        _createHelperTable: function (e, ui) {
            ...
            var hc = ui.is('.stats-slider') || ui.is('.stats-item') ? ui : ui.parents('.stats-slider:first,.stats-item:first');
            ...
            for (i = 0; i < cells.length; i++) {
                ...
                var tr = cell.parents('.stats-row:first');
            ...
            */
            ...
            if (from < to) {
                for (var i = from; i < to; i++) {
                    ...
                    var row2 = trs.find('>.stats-item:nth-child(' + (i + 1) + '),>stats-slider:nth-child(' + (i + 1) + ')')
        ...
        _create: function () {
            ...
            if (el.is('#stats')) {
                ...
                el = this.element = el.find('.stats-row:first');
            }
            else {
                // find parent table
                this._table = el.parents('#stats:first');
            }
            ...
            el.bind('fauxtablestart', this._sortStart());
            el.bind('fauxtableupdate', this._sortUpdate());
            el.bind('fauxtablestop', this._sortStop());
